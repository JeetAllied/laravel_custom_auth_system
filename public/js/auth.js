...
function recaptchaExpireCallbackRegister(response)
{
   ...
   rules: {
    ...
    messages: {
       first_name: {
        ...
        terms: "Please check the terms and conditions check-box.",
        grecaptcha: "please check the I'm not a robot check-box."
    },
    ...
    submitHandler:function(form){
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                ...
                login_validator.resetForm();
                if(status=='422'){
                    var responseData = response.error.customMessages;
...
var register_validator=$('#modal_registration_form').validate({
    ...
    submitHandler:function(form){
        ...
        var formData = new FormData(form);
        $.ajax({
            url: baseUrl+"/auth/ajax_register",
            type: "POST",
            data:formData,
            cache: false,
            contentType: false,
